...
<div id="page-stats" class="page active">
    ...
    <form method="GET" action="{{ url('/statistique') }}" id="filters-form" class="stats-filters">
        <div class="stats-filters-inner">
            ...
            <div class="filter-group" style="flex:1;">
                ...
                <select name="secteur" id="secteur" data-auto>
                    <option value="" {{ $secteurId === 0 ? 'selected' : '' }}>— Tous les secteurs —</option>
                    @foreach($secteurs as $sect)
                    <option value="{{ $sect->id }}" {{ $secteurId === $sect->id ? 'selected' : '' }}>{{ $sect->nom }}</option>
                    @endforeach
                </select>
            </div>
    ...
    {{-- ═══ Information générale — 4 chiffres clés du périmètre choisi ═══ --}}
    <div class="chart-card" style="margin-top:16px;padding:20px;">
        ...
        <div class="stat-kpis" style="grid-template-columns:repeat(4,1fr);">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ── Icônes d'emoji pour chaque secteur ──
    document.querySelectorAll('.js-sect-emoji').forEach(function(el) {
        var nom = el.dataset.nom;
        if (SECT_ICONS[nom]) el.textContent = SECT_ICONS[nom];
    });
...
<style>
    ...
    .btn-submit { display:inline-flex; align-items:center; gap:8px; padding:9px 18px; font-size:12.5px; font-weight:700; font-family:'DM Sans',sans-serif; color:#fff; background:var(--primary,#267a47); border:none; border-radius:8px; cursor:pointer; transition:.2s; }
    .btn-submit:hover { background:#1d5f39; }
    .sect-emoji { margin-right:4px; }
</style>
@endsection